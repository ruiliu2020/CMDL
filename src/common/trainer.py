...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def _train_epoch(self, sub_train_data, train_data, epoch_idx, loss_func=None):
        ...
        for batch_idx, interaction in enumerate(train_data):
            with torch.autograd.set_detect_anomaly(True):
                ...
                learning_loss = self.model.learning_loss(interaction)
                total_learning_loss = total_learning_loss + learning_loss.item()
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, losses):
        ...
        if isinstance(losses, tuple):
            ...
            train_loss_output = train_loss_output + 'train loss: %.4f' % losses
            # train_loss_output = train_loss_output + ' mi loss: %.4f' % total_mi_loss
            # train_loss_output = train_loss_output + ' learning loss: %.4f' % total_learning_loss
            # train_loss_output = train_loss_output + ' high loss: %.4f' % total_high_loss
            # train_loss_output = train_loss_output + ' low loss: %.4f' % total_low_loss
            # train_loss_output = train_loss_output + ' cl loss: %.4f' % total_cl_loss
            # train_loss_output = train_loss_output + ' bpr loss: %.4f' % total_bpr_loss
        return train_loss_output + ']'

    def fit(self, sub_train_data, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_loss, _ = self._train_epoch(sub_train_data, train_data, epoch_idx)
